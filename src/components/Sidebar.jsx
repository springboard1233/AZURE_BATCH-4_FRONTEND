...
export default function Sidebar({ onSelect }) {
  ...
  return (
    <>
      ...
      <button
        className="lg:hidden fixed top-5 left-5 z-50 text-white bg-gradient-to-br from-blue-600 to-indigo-600 
        p-3 rounded-lg shadow-md focus:outline-none transition-transform duration-300 hover:scale-105"
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:static top-0 left-0 h-full w-64 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 
        text-gray-100 p-5 shadow-xl border-r border-gray-700 flex flex-col justify-between transition-transform duration-500 
        z-40 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* Logo Section */}
        <div>
      ...
      {/* Mobile Overlay (dim background when open) */}
      {isOpen && (
        <div
